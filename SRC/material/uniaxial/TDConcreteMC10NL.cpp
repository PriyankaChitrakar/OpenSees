...
TDConcreteMC10NL::setCreepBasicStrain(double time, double stress, double eo)
{
	...
	for (int i = 1; i <= count; i++) {
		...
		double x = 2.6;
		double y = 2.6;
		runSumA += (PHIB_i[i] * beta * STS_i[i]) * (1 + 1.3 * eta_i[i] * pow((SIG_i[i] / fc / (1 + 0.1 * a_i[i])), 4));//Priyanka: 20241222// to check, going to revert back
		//runSumA += (PHIB_i[i] * beta * STS_i[i]);//Priyanka: 20250404 Linear only
		if (i > 1.0) 
		{
			runSumB += (PHIB_i[i] * beta * STS_i[i - 1]) * (1 + 1.3 * eta_i[i-1] * pow((SIG_i[i - 1] / fc / (1 + 0.1 * a_i[i])), 4));//Priyanka: 20241222
...
TDConcreteMC10NL::setCreepDryingStrain(double time, double stress, double eo)
{
	...
	for (int i = 1; i <= count; i++) {
		...
		runSumA += (PHID_i[i] * beta * STS_i[i]) * (1 + 1.3 * eta_i[i] * pow((SIG_i[i] / fc / (1 + 0.1 * a_i[i])), 4));//Priyanka: 20241222
		if (i > 1.0)
		{
			//runSumB += (PHID_i[i] * beta * STS_i[i - 1]) ;//Priyanka: 20250404 Linear only
			runSumB += (PHID_i[i] * beta * STS_i[i - 1]) * (1 + 1.3 * eta_i[i - 1] * pow((SIG_i[i - 1] / fc / (1 + 0.1 * a_i[i])), 4));//Priyanka: 20241222
		}
...
TDConcreteMC10NL::setPhiBasic(double time, double tp)
{
	...
	if (tp <= 720) { ageFactor = (1796 - tp) / 1556; }
	...
	if (t - tcast < (2.0 - 0.0001)) { //Assumed that concrete can only carry load once hardened at 2 days following casting
		...
		if (ops_Creep == 1) {
			if (fabs(t - TIME_i[count]) <= 0.0001) { //If t = t(i-1), use creep/shrinkage from last calculated time step
				...
				//Reactivating failure curve //2025-11-11
				...
				/*Deactivating failure curve //2025-07-23
				if (eps_total < ((ShortTimeStrainD + 0.002) * (2.137 * a + 1) - 0.0013333 * a - 0.002)) 
				//if (eps_total < ((ShortTimeStrainD + 0.002) * (2.137 * a + 1) - 0.0023333 * a - 0.002)) 
