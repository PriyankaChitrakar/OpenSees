...
TDConcreteMC10NL::setCreepBasicStrain(double time, double stress, double eo)
{
	...
	for (int i = 1; i <= count; i++) {
		...
		double x = 2.6;
		double y = 2.6;
		runSum += (PHIB_i[i] * ShortTimeStrain) * (1 + 2 * eta_i[i] * pow((stress / fc / (1 + 0.1)), 4));
...
TDConcreteMC10NL::setCreepDryingStrain(double time, double stress, double eo)
{
	...
	for (int i = 1; i <= count; i++) {
		...
		double x = 2.6;
		double y = 2.6;
		runSum += (PHID_i[i] * ShortTimeStrain) * (1 + 2 * eta_i[i] * pow((stress / fc / (1 + 0.1 * a_i[i])), 4));
...
TDConcreteMC10NL::setPhiDrying(double time, double tp)
{
	...
	double phiDrying = (Ec / Ecm) * (pow(1 - pow(((tp - phiba) / (tp - phiba + phidb)), 0.5), 0.5)) * (phida * pow(tmtp, 0.3) / pow(tmtp + phidb, 0.5));
	///cout << "\n	         phiDrying: " << phiDrying << ".";
	return phiDrying;
}
//ntosic
