...
TDConcreteMC10NL::setPhiBasic(double time, double tp)
{
	...
	double phiBasic = (Ec / Ecm) * (2*(pow(1 - pow(((tp - phiba) / (tp - phiba + phidb)), 0.5), 0.5))-0.5) * ((2 * pow(tmtp, 0.3) / ((pow(tmtp, 0.3)) + 14))+ (pow(7 / tp, 0.5) * pow(tmtp / (tmtp + 7), 0.5))); //
	return phiBasic;
}
//ntosic
double
TDConcreteMC10NL::setPhiDrying(double time, double tp)
{
	...
	double phiDrying = (Ec / Ecm) * (2*(pow(1 - pow(((tp - phiba) / (tp - phiba + phidb)), 0.5), 0.5))-0.5) * (phida * pow(tmtp, 0.5) / pow(tmtp + phidb, 0.5));
	///cout << "\n	         phiDrying: " << phiDrying << ".";
	return phiDrying;
}
//ntosic
